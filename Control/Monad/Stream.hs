...
Nil       `splus` xs = suspended xs               -- suspending
Single y  `splus` xs = Cons y xs
Cons y ys `splus` xs = Cons y (xs `mplus` ys)     -- interleaving
Susp ys   `splus` xs = suspended (xs `mplus` ys)  -- suspending, using `mplus`
